...
export function InventoryStats({ freshItems, expiredItems }: InventoryStatsProps) {
    return (
        <div className="grid grid-cols-2 gap-3 px-6 py-4">
            <div className="bg-[var(--card)]/40 backdrop-blur-md border border-white/10 rounded-[2rem] p-5 shadow-2xl relative overflow-hidden group">
                <div className="absolute top-4 right-4 w-2 h-2 rounded-full bg-[var(--primary)] shadow-[0_0_12px_var(--primary)] animate-pulse" />
                <div className="text-2xl font-black text-[var(--primary)] mb-1">{freshItems}</div>
