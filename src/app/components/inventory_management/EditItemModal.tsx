...
export function EditItemModal({ item, onSave, onDismiss }: EditItemModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-[var(--background)]/90 backdrop-blur-xl flex items-center justify-center p-6">
            <div className="bg-[var(--card)] w-full max-w-sm rounded-[2.5rem] p-6 border border-white/10 shadow-2xl relative">
                ...
                <div className="w-12 h-12 bg-[var(--primary)]/10 rounded-2xl flex items-center justify-center mb-6">
                    <Edit2 size={24} className="text-[var(--primary)]" />
                </div>
                ...
                <div className="space-y-4 mb-8">
                    <div>
                        ...
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-[var(--background)] border border-white/10 rounded-xl px-4 py-3 text-white font-bold focus:border-[var(--primary)] outline-none transition-colors" />
                    </div>
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {["蔬菜", "水果", "肉類", "乳製品", "五穀", "其他"].map(c => (
                                <button key={c} onClick={() => setCategory(c)} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase border transition-all ${category === c ? 'bg-[var(--primary)] text-[var(--background)] border-[var(--primary)]' : 'bg-white/5 text-gray-400 border-white/10'}`}>{c}</button>
                            ))}
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="flex bg-[var(--background)] p-1 rounded-xl border border-white/10">
                        ...
                        <div>
                            ...
                            <input type="number" min="0" value={expiryDays} onChange={(e) => setExpiryDays(parseInt(e.target.value) || 0)} className="w-full bg-[var(--background)] border border-white/10 rounded-xl px-4 py-3 text-white font-bold focus:border-[var(--primary)] outline-none transition-colors text-center" />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <button onClick={handleConfirmSave} className={`w-full py-4 rounded-xl font-black text-sm uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 ${confirmingZero ? 'bg-red-500 text-white' : 'bg-[var(--primary)] text-[var(--background)]'}`}>
