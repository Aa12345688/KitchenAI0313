            ...
            <div className="space-y-10">
                {steps.map((step, index) => (
                    <div key={index} className="relative pl-10">
                        <div className="absolute left-0 top-0 w-6 h-6 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-[var(--primary)] text-[9px] font-black">
                            {index + 1}
                        </div>
                        {index < steps.length - 1 && (
                            <div className="absolute left-3 top-6 bottom-[-40px] w-[1px] bg-gradient-to-b from-[var(--primary)]/20 to-transparent" />
                        )}
