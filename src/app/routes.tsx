...
export const router = createHashRouter([
  {
    ...
    children: [
      {
        index: true,
        element: <ScannerPage />, // 首頁預設：條碼/影像感測頁面
      },
      {
        path: "ingredients",
        element: <IngredientsPage />, // 單一食材與辨識細節
      },
      {
        path: "recipes",
        element: <RecipesPage />, // AI 食譜推薦清單
      },
      {
        path: "recipe/:id",
        element: <RecipeDetailPage />, // 特定食譜的詳細步驟
      },
      {
        path: "saved",
        element: <SavedPage />, // 全域保存庫與資料統計 (Data Statistics)
      },
      {
        path: "profile",
        element: <ProfilePage />, // 使用者個人與系統設定
      },
      {
        path: "inventory",
        element: <InventoryPage />, // 食材庫存清單管理 (可切換冷藏/冷凍)
      },
    ],
  },
]);